...
uint8_t *usart_tx_buf = NULL;
uint16_t usart_rx_len = 0;
uint16_t usart_tx_len = 0;
bool flag_tx_complete = true;
...
{
    if(flag_tx_complete == false) {
        return false;
    }
    else {
        flag_tx_complete = false;
        usart_tx_buf = buf;
        usart_tx_len = len;
...
//------------------------------------------------------------------------------
bool usart_dma_send(uint8_t len)
{
    if(DMA_GetCurrDataCounter(DMA1_Channel4) != 0) return false;
	DMA_Cmd(DMA1_Channel4, DISABLE);
